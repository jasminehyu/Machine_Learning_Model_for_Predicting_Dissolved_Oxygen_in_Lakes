...
class Utils:
    ...
    def plot_predictions(self, df, eval_dict,T, split='Train'):
        ...
        ax.plot(x_plot, plot_gt, linestyle='--', label='Observed_DO',color='#009E73')

        predictions_df=pd.DataFrame(index=x_plot)

        for t in T:
            y_axis = plot_df.loc[t,:].values
            ax.plot(x_plot, y_axis, linestyle='-', label='Predictions',color='#D55E00')

            predictions_df['T'+'+str(t)']=y_axis
        

        
        ax.set_xlabel('Timeline')
        ax.set_ylabel('Dissolved Oxygen Predictions')
        plt.title(f'Dissolved Oxygen Performance on {split} data')
        every_nth = 80
        for n, label in enumerate(ax.xaxis.get_ticklabels()):
            if n % every_nth != 0:
                label.set_visible(False)

        ax.set_xticklabels(x_plot, rotation=75)
        plt.legend()
        plt.show()
